# Council archive boundary

`archive.ts` replays saved `council-v1` events through the unchanged shared
...
The manager keeps archives and active trajectory engines separate. Mutation
routes require an active engine and return HTTP 409 for an archive. Cloning
intake is permitted because it creates a separate trajectories-v2 project;
no historical claims, verdicts, or threads are copied into the new run.

Council scheduling, worker packet composition, model-controller prompts,
