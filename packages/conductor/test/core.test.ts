...
describe("academic model-facing language", () => {
  ...
  it("gives mathematical authors one rigorous ASD-STE100 and TeX contract", () => {
    const state = readyState();
    const decision = decisionPacketFiles(state, {
      digest: null,
      docket: null,
    ...
    for (const contract of [
      ...
      expect(contract).toContain(ASD_STE100_GUIDANCE);
      expect(contract).toContain(MATHEMATICAL_WRITING_GUIDANCE);
      expect(contract).toContain("ASD-STE100 Simplified Technical English (STE), Issue 9");
      expect(contract).toContain("They are examples, not a replacement for the standard");
      expect(contract).toContain("Mathematical rigor and self-containedness have priority");
      expect(contract).toContain("Never weaken,\nstrengthen, or shorten a statement");
      expect(contract).toMatch(/\\left\s+and \\right/);
